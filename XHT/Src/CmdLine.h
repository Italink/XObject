...
private:
    ...
    public:
        ...
        virtual bool parse(std::ostream&, std::ostream&) {
            try {
                value = CmdParser::parse(arguments, value);
                return true;
            }
            catch (...) {
                return false;
...
public:
    explicit CmdParser(int argc, const char** argv) :
        _appname(argv[0]) {
    ...
    CmdParser(int argc, const char** argv, std::string generalProgramDescriptionForHelpText) :
        _appname(argv[0]),
    ...
    CmdParser(int argc, char** argv, std::string generalProgramDescriptionForHelpText) :
        _appname(argv[0]),
        _general_help_text(std::move(generalProgramDescriptionForHelpText)) {
        for (int i = 1; i < argc; ++i) {
            _arguments.push_back(argv[i]);
...
#pragma warning(pop)
    ...
    inline void run_and_exit_if_error() {
        if (run() == false) {
            fprintf(stderr, "HeaderTool: Error command");
            exit(1);
        }
    }
...
protected:
    ...
    std::string usage() const {
        ...
        for (const auto& command : _commands) {
            ...
            if (command->required == false && command->name != "h") {
